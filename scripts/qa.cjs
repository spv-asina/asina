...
(async()=>{
 ...
 const mascotBox=await page.locator('.mascot-button').boundingBox();const mx=mascotBox.x+mascotBox.width/2,my=mascotBox.y+mascotBox.height*.43;for(const [x,y,expected] of [[mx-280,my,'left'],[mx+280,my,'right'],[mx,my-210,'up'],[mx,my+210,'down']]){await page.mouse.move(x,y);await page.waitForTimeout(200);assert.equal(await page.locator('.mascot-button').getAttribute('data-gaze'),expected,`gaze sector ${expected}`);}report.checks.push('Cardinal eye directions match cursor sectors');
 ...
 await page.mouse.move(10,10);await page.waitForTimeout(200);const left=await page.locator('.mascot-button').getAttribute('data-gaze');await page.mouse.move(1400,700);await page.waitForTimeout(200);const right=await page.locator('.mascot-button').getAttribute('data-gaze');assert.notEqual(left,right);assert.match(left,/^(up|down|left|right|up-left|up-right|down-left|down-right|center)$/);assert.match(right,/^(up|down|left|right|up-left|up-right|down-left|down-right|center)$/);await page.locator('.mascot-button').click();assert.equal(await page.locator('.toast').isVisible(),true);report.checks.push('Mascot follows cursor by 9 gaze sectors and responds to click');
 ...
 await page.emulateMedia({reducedMotion:'reduce'});await page.goto(base);assert.equal(await page.locator('.chibi-eyes').evaluate(e=>getComputedStyle(e).animationName),'none');await page.mouse.move(10,10);await page.waitForTimeout(100);assert.equal(await page.locator('.mascot-button').getAttribute('data-gaze'),null);report.checks.push('Reduced motion disables mascot animations and tracking');
